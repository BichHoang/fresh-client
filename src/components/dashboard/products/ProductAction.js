import React, { Component } from 'react'
import Header from '../commons/Header'
import Sidebar from '../commons/Sidebar'
import Footer from '../commons/Footer'
import callApi from '../../../api/apiCaller';
import { Link } from 'react-router-dom';

class ProductAction extends Component {
    constructor(props) {
        super(props)
        this.state = {
            id_product: '',
            name: '',
            preview: '',
            detail: '',
            price: '',
            id_cat: '',
            quantity: 50,
            enable: '',
            categories: [],
            imagePreviewUrl: '',
            pic: ''
        }
    }

    onChange = (e) => {
        var target = e.target;
        var name = target.name;
        var value = target.type === 'checkbox' ? target.checked : target.value;
        if (target.type === 'file') {
            this.loadImage(e);
        }
        this.setState({
            [name]: value
        });
    }

    loadImage(e) {
        e.preventDefault();

        let reader = new FileReader();
        let file = e.target.files[0];

        reader.onloadend = () => {
            this.setState({
                pic: reader.result.slice("data:image/jpeg;base64,".length),
                imagePreviewUrl: reader.result,
            });
        }

        reader.readAsDataURL(file)
    }

    onSave = (e) => {
        e.preventDefault();
        var { id_product, name, preview, detail, price, id_cat, quantity, enable, pic } = this.state;
        var { history } = this.props;
        console.log(this.state)
        if(id_product){
            callApi('/admin/product/', 'PUT', {
                id_product: id_product,
                name: name,
                preview: preview,
                detail: detail,
                pic: pic,
                price: price,
                id_cat: id_cat,
                quantity: quantity,
                enable: enable,
            }).then(res => {
                history.goBack();
            })
        }else{
            callApi('/admin/product/', 'POST', {
                name: name,
                preview: preview,
                detail: detail,
                pic: pic,
                price: price,
                id_cat: id_cat,
                quantity: quantity,
                enable: enable,
            }).then(res => {
                history.goBack();
            })
        }
    }

    componentDidMount() {
        var { match } = this.props;
        callApi('admin/categories', 'GET', null).then(res => {
            const categories = res.data;
            this.setState({
                categories: categories,
                id_cat: categories[0].idCat
            });
        })
        if (match) {
            var id = match.params.id;
            callApi(`product/${id}`, 'GET', null).then(res => {
                var data = res.data;
                console.log(data)
                this.setState({
                    id_product: id,
                    name: data.name,
                    preview: data.preview,
                    detail: data.detail,
                    pic: data.pic,
                    price: data.price,
                    id_cat: data.id_cat,
                    quantity: data.quantity,
                    enable: data.enable,
                });
            })
        }
        
    }

    
    findCategoryById = (categories, id) => {
        var result = "Category";
        categories.forEach((category) => {
            if(category.idCat === id){
                result = category.name;
            }
        })
        return result;
    }

    myFormat = (num) => {
        return num + '$';
    }

    render() {

        var { name, preview, detail, pic, price, id_cat, quantity, enable, categories } = this.state;
        let category = categories.map((cat, index) => {
            return (
                <option key={index} value={cat.idCat}>{cat.name}</option>
            )
        })
        let { imagePreviewUrl } = this.state;
        let $imagePreview = null;
        if (imagePreviewUrl) {
            $imagePreview = (<div><img height="80" width="80" src={imagePreviewUrl} /></div>);
        } else {
            $imagePreview = (<img height="80" width="80" src={`data:image/jpeg;base64,${pic}`} />);
        }
        return (
            <div className="container-scroller">
                <Header />
                <div className="container-fluid page-body-wrapper">
                    <Sidebar />
                    <div className="main-panel">
                        <div className="content-wrapper">
                            <div className="content-wrapper">
                                <div className="col-8 offset-2 grid-margin stretch-card">
                                    <div className="card">
                                        <div className="card-body">
                                            <h4 className="card-title">{(this.state.id_product === '' ? 'Thêm mới sản phẩm' : 'Chỉnh sửa sản phẩm')}</h4>
                                            <br />
                                            <form className="forms-sample" onSubmit={this.onSave}>
                                                <div className="form-group row">
                                                    <div className="col-4 offset-4">
                                                        {$imagePreview}
                                                    </div>
                                                </div>
                                                <div className="form-group row">
                                                    <div className="col-4 offset-5">
                                                        <input type="file" onChange={this.onChange} />
                                                    </div>
                                                </div>
                                                <div className="form-group row">
                                                    <label htmlFor="exampleInputUsername2" className="col-sm-3 col-form-label">Name</label>
                                                    <div className="col-sm-9">
                                                        <input type="text" className="form-control"
                                                            name="name"
                                                            id="exampleInputUsername2"
                                                            onChange={this.onChange}
                                                            value={name}
                                                            placeholder="Tên sản phẩm" />
                                                    </div>
                                                </div>
                                                <div className="form-group row">
                                                    <label htmlFor="exampleInputEmail2" className="col-sm-3 col-form-label">Preview</label>
                                                    <div className="col-sm-9">
                                                        <textarea className="form-control"
                                                            name="preview"
                                                            type="text"
                                                            id="exampleTextarea1"
                                                            value={preview}
                                                            onChange={this.onChange}
                                                            rows="3"></textarea>
                                                    </div>
                                                </div>
                                                <div className="form-group row">
                                                    <label htmlFor="exampleInputEmail2" className="col-sm-3 col-form-label">Detail</label>
                                                    <div className="col-sm-9">
                                                        <textarea className="form-control"
                                                            id="exampleTextarea1"
                                                            name="detail"
                                                            onChange={this.onChange}
                                                            value={detail}
                                                            rows="5"></textarea>
                                                    </div>
                                                </div>
                                                <div className="form-group row">
                                                    <label htmlFor="exampleInputMobile" className="col-sm-3 col-form-label">Price</label>
                                                    <div className="col-sm-3">
                                                        <input type="text" className="form-control"
                                                            id="exampleInputMobile"
                                                            name="price"
                                                            onChange={this.onChange}
                                                            value={price}
                                                            placeholder="Price" />
                                                    </div>
                                                    <label htmlFor="exampleInputMobile" className="col-sm-2 col-form-label">Category</label>
                                                    <div className="col-sm-4">
                                                        <select className="form-control form-control-lg"
                                                            id="exampleFormControlSelect1"
                                                            value={id_cat}
                                                            onChange={this.onChange}
                                                            name="id_cat">
                                                            {category}
                                                        </select>
                                                    </div>
                                                </div>
                                                <div className="form-group row">
                                                    <label htmlFor="exampleInputMobile" className="col-sm-3 col-form-label">Quantity</label>
                                                    <div className="col-sm-1">
                                                        <input type="number" id="quantity"
                                                            name="quantity"
                                                            min={0} max={500}
                                                            value={quantity}
                                                            onChange={this.onChange} />
                                                    </div>
                                                </div>
                                                <div className="form-group row">
                                                    <div className="col-2 offset-3">
                                                        <div className="form-check">
                                                            <label className="form-check-label">
                                                                <input
                                                                    name="enable" type="checkbox"
                                                                    className="form-check-input"
                                                                    id="exampleInputName1"
                                                                    onChange={this.onChange}
                                                                    value={enable}
                                                                    checked={enable}
                                                                />
                                                                <i className="input-helper"></i>
                                                                    Active
                                                                </label>
                                                        </div>
                                                    </div>
                                                </div>
                                                <button type="submit" className="btn btn-gradient-primary mr-2">{(this.state.id_product === '' ? 'Create' : 'Update')}</button>
                                                <Link to="/admin/products" className="btn btn-light">Cancel</Link>
                                            </form>
                                        </div>
                                    </div>
                                </div>

                            </div>
                        </div>
                        <Footer />
                    </div>
                    {/* main-panel ends */}
                </div>
            </div >
        )
    }
}

export default ProductAction;